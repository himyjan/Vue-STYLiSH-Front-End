...
const clickDot = (index: number) => {
  ...
  intervalRef.value = window.setInterval(() => {
    activeCampaignIndex.value =
      activeCampaignIndex.value === campaigns.value.length - 1 ? 0 : activeCampaignIndex.value + 1;
  }, 5000);
}
</script>
...
<template>
  <Wrapper>
    ...
    <Dots>
      <Dot :isActive="(index === activeCampaignIndex)" :key="index" @click="() => clickDot(index)"
